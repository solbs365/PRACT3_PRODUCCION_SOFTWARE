...
def test_create_multiple_expenses_and_list():
    ...
    service.create_expense("Pan", 3, "Mercado", date(2020, 1, 5))
    service.create_expense("Leche", 4, "Supermercado", date(2021, 10, 6))

    # Se obtiene listado
    expenses = service.list_expenses()

    # Verificación:
    # 1. Comprobar que el tamaño sea 2
...
def test_remove_expense_reduces_total():
    ...
    service.create_expense("Libro", 15, "", date(2020, 2, 3))
    ...
    # eliminamos el gasto con ese id
    service.remove_expense(expense_to_remove)
...
def test_update_expense_partial_fields():
    ...
    id_expense = service.list_expenses()[0].id

    # Lo actualizamos
    service.update_expense(id_expense, "Camiseta", 18, "Ropa")

    # Verificaciones
    service_list = service.list_expenses()[0]
    assert service_list.title == "Camiseta"
    assert service_list.description == "Ropa"
    assert service_list.amount == 18
...
def test_total_amount_after_removal():
    ...
    service.create_expense("Cursos", 30, "", date(2026, 3, 4))
    service.create_expense("Internet", 25, "", date(2026, 3, 4))
